...
export default function Dashboard() {
  ...
  // Fetch stock alerts
  const { data: stockAlertsData, error: stockError } = useSWR('/api/stock?action=alerts', fetcher, { 
    refreshInterval: 60000, // تحديث كل دقيقة
    revalidateOnFocus: true 
  });

  const stockAlerts = stockAlertsData?.alerts || [];
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 sm:p-6">
            <div className="flex items-start space-x-3 mb-4">
              ...
              <div>
                <h3 className="text-lg font-semibold text-yellow-900 mb-2">
                  تحذير: منتجات تحتاج إلى تجديد المخزون
                </h3>
                <p className="text-yellow-800 text-sm mb-4">
                  المنتجات التالية وصلت إلى الحد الأدنى أو أقل من 10 قطع:
                </p>
              </div>
            </div>
                  ...
                  <div className="space-y-1 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className={`font-bold ${
                        item.currentQuantity <= 0 ? 'text-red-600' : 'text-yellow-600'
                      }`}>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-gray-800">{item.minThreshold || 10} قطعة</span>
                    </div>
                    
                    <div className="flex justify-between">
                      <span className="text-gray-600">الكمية الأولية:</span>
                      <span className="text-gray-800">{item.initialQuantity} قطعة</span>
                    </div>
                    
                    {item.currentQuantity > 0 && item.currentQuantity < (item.minThreshold || 10) && (
                      <div className="mt-2 p-2 bg-yellow-50 rounded text-xs text-yellow-700">
                        💡 يحتاج إضافة {(item.minThreshold || 10) - item.currentQuantity + 10} قطعة على الأقل
                      </div>
                    )}
                    
                    {item.currentQuantity <= 0 && (
                      <div className="mt-2 p-2 bg-red-50 rounded text-xs text-red-700">
                        🚨 المنتج نفد تماماً - يحتاج تجديد فوري!
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
              <div className="flex items-center space-x-2 mb-2">
                ...
                <span className="font-semibold text-blue-900 text-sm">توصيات:</span>
              </div>
              <ul className="text-blue-800 text-sm space-y-1 list-disc list-inside">
                <li>انتقل إلى تبويب "إدارة المخزون" لإضافة مخزون جديد</li>
                <li>تواصل مع الموردين لطلب كميات إضافية</li>
                <li>راجع معدل البيع لتحديد الكمية المطلوبة</li>
                <li>فكر في رفع الحد الأدنى للمنتجات سريعة البيع</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        {stockAlerts.length > 0 && (
          <div className="mt-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
              ...
              <StatCard 
                label="منتجات نفدت" 
                value={stockAlerts.filter((item: any) => item.currentQuantity <= 0).length} 
                icon="🚨" 
                bgColor="bg-red-50" 
              ...
              <StatCard 
                label="بحاجة لإجراء" 
                value={stockAlerts.filter((item: any) => item.currentQuantity <= (item.minThreshold || 10) / 2).length} 
                icon="🔥" 
                bgColor="bg-purple-50" 
